  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-4">
      <form onSubmit={handleSubmit} className="relative group z-20">
        <div
          className={`absolute -inset-0.5 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-2xl opacity-20 group-hover:opacity-40 transition duration-500 ${isFocused ? "opacity-60" : ""}`}
        ></div>
        <div className="relative flex items-center bg-dark-800 border border-dark-600 rounded-2xl p-2 transition-all shadow-xl group-hover:border-dark-500">
          <Search
            className={`ml-3 w-5 h-5 transition-colors ${isFocused ? "text-cyan-400" : "text-gray-500"}`}
          />
          <input
            ref={inputRef}
            type="text"
          ...
          <div className="hidden md:flex items-center gap-3 mr-2">
            ...
            <div className="flex items-center gap-2 text-[10px] text-dark-400 font-bold uppercase tracking-widest border-l border-dark-700 pl-3">
              <kbd className="bg-dark-700/50 px-1.5 py-0.5 rounded border border-dark-600">
                /
              </kbd>
              <span>Focus</span>
            </div>
