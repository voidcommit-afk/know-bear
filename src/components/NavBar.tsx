...
export function NavBar(): JSX.Element {
  ...
  return (
    <div className="absolute top-0 left-0 w-full p-4 flex justify-end items-start pointer-events-none z-50">
      <div
        className={`pointer-events-auto flex items-center gap-3 px-1.5 py-1.5 rounded-full transition-all duration-300 ${!user ? "" : "bg-dark-800/80 backdrop-blur-md border border-dark-700 shadow-lg"}`}
      >
        {!user ? (
          ...
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="flex flex-col pr-2">
              <span
                className={`text-xs font-bold leading-none ${isPro ? "text-amber-400" : "text-gray-200"}`}
              >
