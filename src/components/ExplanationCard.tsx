...
const markdownComponents: Components = {
  code({ className, children, inline, ...props }: MarkdownCodeProps) {
    ...
    return (
      <code
        className={`${className} bg-dark-900 rounded px-1.5 py-0.5 text-xs font-mono`}
        {...props}
      >
        {children}
      </code>
  ...
  pre({ children }: ComponentPropsWithoutRef<"pre">) {
    return (
      <pre className="bg-dark-900 p-4 rounded-xl border border-white/5 overflow-x-auto my-4">
        {children}
      </pre>
    );
  },
  ...
  a({ ...props }: MarkdownAnchorProps) {
    return (
      <a
        ...
        className="underline decoration-cyan-500/30 underline-offset-4 hover:decoration-cyan-400 transition-all font-medium"
      />
    );
  },
  h2({ children }: MarkdownHeadingProps) {
    return (
      <h2 className="text-2xl font-bold mt-8 mb-4 text-white border-b border-white/5 pb-2">
        {children}
      </h2>
    );
  },
  h3({ children }: MarkdownHeading3Props) {
    return (
      <h3 className="text-xl font-semibold mt-6 mb-3 text-cyan-100">
        {children}
      </h3>
    );
  },
